...
export function createArticles({ar, lang, chapters, icon, num, footer, path, fiches, illustrationUI}) {
  let articles;
  let pending;
  const words = ar ? {
  ...
  async function renderArticle(n,target) {
    ...
    try {
      ...
      if(location.href!==requestUrl)return;
      ...
      target.innerHTML=`<div class="breadcrumb"><a href="${path({type:'chapter',id:c.id,mode:'details'})}">${words.back}</a>${icon('chevron','directional')}<span>${words.verse} ${num(n)}</span></div><article class="detailed-article"><header class="article-heading"><span class="eyebrow">${words.verse} ${num(n)}</span><h1>${esc(entry.title)}</h1>${copyButton('article',n,words.copyArticle)}</header><section class="paper article-verse">${verse(v)}</section><div class="article-body">${entry.sections.map(s=>`<section class="article-section"><h2>${esc(s.title)}</h2>${s.paragraphs.map(p=>`<p>${referenceText(p,a)}</p>`).join('')}</section>`).join('')}</div><aside class="article-references"><h2>${words.references}</h2><ol>${a.references.map(r=>`<li><a href="${esc(r.url)}" target="_blank" rel="noopener noreferrer">${esc(r[lang])}</a>${(r.links||[]).map(p=>` <a class="source-page" href="${esc(p.url)}" target="_blank" rel="noopener noreferrer">${esc(p[lang])}</a>`).join('')}</li>`).join('')}</ol><p>${words.method}</p></aside></article><nav class="chapter-pagination" aria-label="${words.label}">${n>1?`<a href="${path({type:'article',verse:n-1})}">${icon('arrow','directional back')}${words.prev}</a>`:'<span></span>'}${n<allVerses.length?`<a class="next" href="${path({type:'article',verse:n+1})}">${words.next}${icon('arrow','directional')}</a>`:''}</nav>${footer()}`;
    } catch {
      if(location.href!==requestUrl)return;
  ...
  document.addEventListener('click',async e=>{
    ...
    try {
      ...
      if(kind==='fiche'){const f=fiches.find(f=>f.id===button.dataset.fiche);if(f)text=ficheText(f,lang,location.origin+path({type:'fiche',fiche:f.id}));}
      if(kind==='brief'){const c=getChapter(n);text=c?.[ar?'bodyAr':'bodyFr'];}
      if(kind==='article'){const data=await load(),a=data.find(a=>a.n===n);if(a)text=articleText(a,getVerse(n),lang,location.origin+path({type:'article',verse:n}));}
